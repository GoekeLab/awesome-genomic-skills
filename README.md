...
- [Noodle Biomedical Literature Discovery MCP](https://github.com/helena-bioinformatics/noodle-mcp) - Public, read-only Helena Bioinformatics MCP and companion Agent Skill for natural-language or PMID/DOI/PMCID paper discovery, source-linked publication records, and bounded citation or semantic literature-graph traversal with provenance; hosted Streamable HTTP endpoint with no authentication or PHI.
