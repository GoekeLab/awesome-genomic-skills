...
- [DeepDNA MCP](https://deepdna.ai/docs/#mcp) - Hosted, read-only MCP server (`https://deepdna.ai/mcp`, no key, Streamable HTTP) with cited gene, variant (rsID) and CPIC/DPWG drug–gene lookups for pharmacogenomics: `lookup_gene`, `lookup_variant`, `lookup_drug_gene`. Small curated dataset (12 genes, mostly pharmacogenes), every record with its sources; listed in the official MCP registry as `ai.deepdna/knowledge`.
